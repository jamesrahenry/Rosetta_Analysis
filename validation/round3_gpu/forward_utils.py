...
def require_cuda_if_expected(model_id: str = "") -> None:
    """Fail fast when the GPU has silently vanished.

    The rosetta-runner container occasionally loses GPU passthrough mid-session
    (``torch.cuda.is_available()`` flips to False while the host ``nvidia-smi``
    is still fine). Without this guard, ``get_device()`` falls back to CPU and a
    large model then grinds for HOURS in fp32 on CPU (cost this a full day once,
    2026-07-19). On the GPU host the image sets ``ROSETTA_REQUIRE_CUDA=1`` so
    this raises immediately instead. Recover by restarting the container
    (``systemctl restart rosetta-runner.service`` — re-establishes ``--gpus
    all``); checkpoint shards make the resume cheap. No-op unless the env var is
    set, so CPU/laptop/smoke runs are unaffected.
    """
    import os
    if os.environ.get("ROSETTA_REQUIRE_CUDA", "") not in ("1", "true", "True"):
        return
    import torch
    if not torch.cuda.is_available():
        raise RuntimeError(
            f"ROSETTA_REQUIRE_CUDA=1 but torch.cuda.is_available()==False while "
            f"loading {model_id!r}: the container has lost GPU passthrough (the "
            f"host's nvidia-smi may still see the GPU). Refusing to run "
            f"fp32-on-CPU. Recover with `systemctl restart rosetta-runner.service` "
            f"on the host, then resume — per-model checkpoint shards skip completed work.")


def load_model(model_id: str):
    """bf16 (or best available) causal LM + tokenizer, eval mode."""
    _allow_torch_bin_load()
    require_cuda_if_expected(model_id)
