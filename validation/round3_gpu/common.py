...
PAPER_REVISION = "paper-n250"                    # frozen tag: caz_/gem_ live here (main can lag)
...
def load_caz(slug: str, concept: str, root: Path = MODELS_ROOT) -> dict:
    ...
    Self-healing: if the file isn't present under MODELS_ROOT (fresh host, or a
    dispatch-hook job that bypassed download_phase), fetch it from HF instead of
    raising FileNotFoundError. The frozen caz_ files live on the ``paper-n250``
    revision (``main`` may carry gem_ only), so try that first, then ``main``.
    """
    p = root / slug / f"caz_{concept}.json"
    if not p.exists():
        _fetch_artifact_from_hf(slug, f"caz_{concept}.json", root)
    return json.loads(p.read_text())


def _fetch_artifact_from_hf(slug: str, filename: str, root: Path = MODELS_ROOT) -> None:
    """Download one paper_n250 artifact into MODELS_ROOT/<slug>/ (paper-n250 rev,
    falling back to main). Lets hook jobs run on a fresh host with no manual
    'restore on a new node' step."""
    import shutil
    from huggingface_hub import hf_hub_download
    repo_path = f"{PAPER_TREE}/{slug}/{filename}"
    last_err: Exception | None = None
    for rev in (PAPER_REVISION, "main"):
        try:
            src = hf_hub_download(HF_DATASET, repo_path, repo_type="dataset", revision=rev)
            dst = root / slug / filename
            dst.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy(src, dst)
            log.info("[artifact] fetched %s from HF (%s)", repo_path, rev)
            return
        except Exception as e:  # noqa: BLE001 — try next revision
            last_err = e
    raise FileNotFoundError(
        f"{repo_path} not found locally and not fetchable from HF "
        f"(tried {PAPER_REVISION}, main): {last_err}")
...
def hf_download_artifacts(patterns: list[str], local_dir: Path = ROSETTA_DATA) -> None:
    ...
    Uses the ``paper-n250`` revision: the frozen caz_ files live there, whereas
    ``main`` (the snapshot_download default) carries gem_ only — pulling from
    main silently returned 0 caz files and broke every hook on a fresh host."""
    from huggingface_hub import snapshot_download
    snapshot_download(
        HF_DATASET, repo_type="dataset", revision=PAPER_REVISION, local_dir=str(local_dir),
