...
# Local ModelScope cache paths — bypasses HF download for these models.
...
LOCAL_MODEL_PATHS: dict[str, Path] = {
    ...
    "google/gemma-2-9b-it":      MODELSCOPE_ROOT / "google"    / "gemma-2-9b-it",
...
def run_model(model_id: str, concepts: list[str], args) -> None:
    ...
    if model_id in LOCAL_MODEL_PATHS:
        ...
        try:
            model = AutoModelForCausalLM.from_pretrained(load_path, torch_dtype=dtype, device_map=device)
        except (ValueError, ImportError):
            model = AutoModelForCausalLM.from_pretrained(load_path, torch_dtype=dtype).to(device)
    else:
