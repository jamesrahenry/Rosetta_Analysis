"""
aggregate_random_control_handoff.py — Summarise handoff-layer random-direction
ablation results (GEM paper §5.3's direction-specificity control).

Handoff-layer counterpart to aggregate_random_control.py, which aggregates the
CAZ-*peak*-layer control (`ablation_random_<concept>.json`) for the CAZ
Validation paper's §6.1. This script aggregates the handoff-layer control
(`ablation_random_handoff_<concept>.json`, produced by the
gem_random_direction_handoff Prefect flow in eigan-ai/infra, not by
ablate_random_direction.py in this repo) for the GEM paper's §5.3.

Differences from the peak-layer version:
  - Reads `ablation_random_handoff_*.json`, not `ablation_random_*.json`.
  - Adds Phi-2 as its own cohort rather than folding it into MHA, matching
    GEM preprint.md §5.1's Appendix A convention of treating phi-2 as a
    parallel-attention architecture excluded from the MHA/GQA split.
  - Reads `paper_n250/_gem_random_handoff/<model>/*.json` layout, not the
    per-model extraction-dir layout `ablate_random_direction.py` writes.

Produces:
  <out-dir>/random_ablation_control_handoff.json  — full data + summary
  <out-dir>/fig_random_control.png                — paper Figure 2
  <out-dir>/random_ablation_table_handoff.txt      — cohort table for §5.3

Usage
-----
    python gem/aggregate_random_control_handoff.py
    python gem/aggregate_random_control_handoff.py --data-dir /path/to/_gem_random_handoff --out-dir /path/to/out
...
DEFAULT_DATA_DIR = Path.home() / "rosetta_data" / "paper_n250" / "_gem_random_handoff"
CAZ_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_OUT_DIR = Path.home() / "rosetta_data" / "results" / "random_control_handoff"

COHORT_MAP = {
    "Pythia": "MHA", "GPT-2": "MHA", "OPT": "MHA",
    "Qwen":   "GQA", "Llama": "GQA", "Mistral": "GQA",
    "Gemma":  "Gemma",
    "Phi":    "Phi-2",
}
COHORT_ORDER  = ["MHA", "GQA", "Gemma", "Phi-2"]
COHORT_COLORS = {"MHA": "#C62828", "GQA": "#1565C0", "Gemma": "#00695C", "Phi-2": "#4E342E"}
...
def load_records(data_dir: Path) -> list[dict]:
    records = []
    files = list(data_dir.rglob("ablation_random_handoff_*.json"))
    log.info("Found %d handoff-layer random-ablation files under %s", len(files), data_dir)
    for f in sorted(files):
        ...
        records.append({
            ...
            "z_score":     float(d.get("z_score")) if d.get("z_score") is not None else None,
...
def summary_table(records: list[dict]) -> dict:
    base = [r for r in records if not r["is_instruct"] and r["cohort"] != "Unknown"
            and r["concept_red"] is not None and r["concept_red"] > 0]
    if not base:
        return {}
...
def print_summary(s: dict) -> None:
    ...
    o = s["overall"]
    print("=" * 72)
    print("HANDOFF-LAYER RANDOM-DIRECTION ABLATION CONTROL — SUMMARY (base models only)")
    print("=" * 72)
...
def write_table(s: dict, out_path: Path) -> None:
    lines = [
        "Handoff-layer random-direction ablation control — cohort summary (base models only)",
...
def plot_figure(records: list[dict], summary: dict, out_path: Path) -> None:
    ...
    fig, (ax_violin, ax_bar) = plt.subplots(1, 2, figsize=(13, 5))
    ...
    ax_violin.set_title("Random seed distribution vs concept direction (dots), handoff layer",
    ...
    ax_bar.set_title("Direction specificity by architecture cohort, handoff layer",
                     fontsize=10, fontweight="bold", color=THEME["text"], loc="left")
    ...
    fig.suptitle(
        "Concept-direction ablation at the GEM handoff layer is specific: random directions "
        f"produce ~{summary['overall']['mean_random_red']:.3f} reduction vs "
...
def main() -> None:
    ...
    ap.add_argument("--data-dir", type=Path, default=DEFAULT_DATA_DIR,
                     help="Directory containing <model>/ablation_random_handoff_<concept>.json")
    ...
    if not records:
        log.error("No ablation_random_handoff_*.json files found under %s.", args.data_dir)
        sys.exit(1)

    summary = summary_table(records)
    print_summary(summary)

    out_json = args.out_dir / "random_ablation_control_handoff.json"
    out_json.write_text(json.dumps({"summary": summary, "records": records}, indent=2))
    log.info("Saved %s (%d records)", out_json, len(records))

    plot_figure(records, summary, args.out_dir / "fig_random_control.png")
    write_table(summary, args.out_dir / "random_ablation_table_handoff.txt")
