...
def run_model(model_id: str, concepts: list[str], args) -> list[dict]:
    ...
    try:
        ...
        if model_id in LOCAL_MODEL_PATHS:
            load_path = str(LOCAL_MODEL_PATHS[model_id])
            log.info("Loading %s from local path: %s", model_id, load_path)
            tokenizer = AutoTokenizer.from_pretrained(load_path)
            try:
                model = AutoModel.from_pretrained(load_path, torch_dtype=dtype, device_map=device)
            except (ValueError, ImportError):
                model = AutoModel.from_pretrained(load_path, torch_dtype=dtype).to(device)
        else:
    ...
    release_model(model)
    if not args.no_clean_cache and model_id not in LOCAL_MODEL_PATHS:
...
def main():
    ...
    parser.add_argument("--local-path", default="",
                        help="Override local weight path for --model (e.g. /tmp/Mistral-7B-v0.3)")
